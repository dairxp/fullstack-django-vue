...
#             return JsonResponse({"estado":"error", "mensaje": "El usuario no existe"}, status=HTTPStatus.NOT_FOUND)

#         auth = authenticate(request, username=request.data.get("correo"), password=request.data.get("password"))
#         if auth is not None:

#             return JsonResponse({"estado": "ok", "mensaje": "Login exitoso"})
#         else:
#             return JsonResponse({"estado":"error", "mensaje": "Las credenciales ingresadas no son correctas"}, status=HTTPStatus.NOT_FOUND)
